...
class QLDBillList(BillListExtractor):
    ...
    def _get_all_bills(self):
        ...
        parl_number = (
            landing_page.find(
                "table", {"class": "table table-bordered table-condensed browse-table"}
            )
            .find("td")
            .text[:2]
        )  # Current parliament's number

        api_json = self._download_json(API_CALL.format(parl_number, self._get_epoch()))
...
class QLDBillHelper(BillExtractor):
    def __init__(self, bill_meta: BillMetaQLD):
        ...
        self.history_page = self._download_html(self.url + "/lh").find(
