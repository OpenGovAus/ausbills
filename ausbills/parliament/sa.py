...
class SABillList(BillListExtractor):
    ...
    def _get_all_bills(self):
        table = self._download_html(BASE + "listBills.aspx?key=").find(
            "tbody"
        )  # By providing a null key, the table returns all values.
        ...
        for bill_row in table.find_all("tr"):
            ...
            bills_list.append({TITLE: bill_title, URL: bill_url, SPONSOR: sponsor})
        return bills_list
...
class SAHelper(BillExtractor):
    ...
    def _get_text_links(self):
        ...
        for index, row in enumerate(self.bill_soup.find_all("tr")):
            link = BASE + row.find_all("td")[1].find("a")["href"].replace(" ", "%20")

            house = (
                House.LOWER.value
                if "House of Assembly" in row.text
                else House.UPPER.value
            )

            links.append({API_ID: index, API_HOUSE: house, URL: link})
        return links
    ...
    def _get_progress(self):
        ...
        if (
            "received" in final_row.text
        ):  # This means that the bill has been moved from one house to another
            prog_dict = {
                BillProgress.FIRST.value: True,
