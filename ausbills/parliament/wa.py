...
class WABillList(BillListExtractor):
    ...
    def _get_bills(self):
        ...
        for row in table.find_all("tr"):
            bill_intro = self._get_timestamp(
                row.find_all("td")[2].text.strip(), "%d %b %Y"
            )
...
class WABillHelper(BillExtractor):
    ...
    def _get_text_links(self, timestamp):
        try:
            ...
            wa_logger.warn(
                "An error occurred when obtaining the bill introduction text;\n "
                + str(e)
            )
            return [{}]
    ...
    def _get_pdf(self, event_label):
        return (
            BASE_URL + self.bill_soup.find("a", {"ga-event-label": event_label})["href"]
        )
    ...
    def _get_speech(self):
        ...
        try:
            ...
            wa_logger.debug(
                "No Legislative Asembly second reading speech found.\n" + str(e)
            )
        ...
        try:
            ...
            wa_logger.debug(
                "No Legislative Council second reading speech found.\n" + str(e)
            )
    ...
    def _get_em_statement(self):
        ...
        try:
            statements.append(
                {
                    URL: self._get_pdf("Explanatory Memorandum LA"),
                    API_HOUSE: BillProgress.FIRST.value,
                }
            )
        except Exception as e:
        ...
        try:
            statements.append(
                {
                    URL: self._get_pdf("Explanatory Memorandum LC"),
                    "house": BillProgress.SECOND.value,
                }
            )
        except Exception as e:
        ...
        for index, statement in enumerate(statements):
            _compiled_list.append(
                {API_ID: index, URL: statement[URL], API_HOUSE: statement[API_HOUSE]}
            )
        return _compiled_list
    ...
    def _get_reading(self):
        def __get_num(text):
            if "First" in text:
                ...
                return (
                    ChamberProgress.THIRD_READING.value
                )  # Even if the bill has passed, we should return that the third reading was reached.

        return __get_num(
            self.table[0].find_all("tr")[-1].find_all("td")[-1].contents[0]
        )
