...
class NTBillList(BillListExtractor):
    ...
    def __get_all_bills(self):
        ...
        table = webpage.find('table', {'class': 'table'})  # Finds the first table on the page, which is the most recent (current) parliament session.
...
class BillNTHelper(BillExtractor):
    ...
    def __get_chamb_progress(self):
        return ChamberProgress.SECOND_READING.value \
            if not self.passed else ChamberProgress.THIRD_READING.value
    ...
    def __get_pdf(self):
        pdfURL =  self.bill_soup.find('i', {'class': 'fa fa-file-pdf-o fa-3x'}).parent['href']
        return [{
            API_ID: 0,
    ...
    def __get_em_links(self):
        try:
            pdfUrl = self.bill_soup.find('i', {'class': 'fa fa-file-pdf-o fa-2x'}).parent['href']
            return [{
                API_ID: 0,
                API_HOUSE: House.LOWER.value,
                API_TIME: self.intro_date,
