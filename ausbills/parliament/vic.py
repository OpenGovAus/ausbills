...
API_CALL = (
    BASE + "app/api/search/title_content?from=0&size=999&include"
...
class VicBillList(BillListExtractor):
    ...
    def _get_all_bills(self):
        ...
        for bill_dict in json_bills:
            ...
            bills_list.append(
                {TITLE: bill_title, URL: bill_url, PROGRESS: bill_progress}
            )
        return bills_list
...
class VicBillHelper(BillExtractor):
    ...
    def __repr__(self):
        return "<{}.{} : {} object at {}>".format(
            self.__class__.__module__, self.__class__.__name__, self.url, hex(id(self))
        )
    ...
    def _get_doc_url(self, data_tid):
        header = self.bill_soup.find("li", {"data-tid": data_tid})
        if header is not None:
            for link in header.find_all("a"):
                if ".pdf" in link["href"] or ".PDF" in link["href"]:
                    doc_url = link["href"]

            return [{URL: doc_url, API_ID: 0}]
        else:
            return [{}]
